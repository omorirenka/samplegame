class MyScene extends Phaser.Scene {
   ...
   create() {
       ...
       this.hanako_Text = this.add.text(100, 50, '', { fontSize: '28px', fill: '#FFF' ,fontFamily: "Arial"});
       // let image_x = Phaser.Math.Between(50, 750) ; 
       // let image_y =  Phaser.Math.Between(50, 200) ; 
...
update(time, delta) {
   ...
   if (this._timeCounter >= 3000) {
       ...
       if (this.time === 3) {
           ...
           hanako.children.iterate(function (hanako) {
               let image_x = Phaser.Math.Between(200, 400);
               let image_y = Phaser.Math.Between(100, 200);
               hanako.setPosition(image_x, image_y);
           });
       }
   ...
   wasd_move(keys, object){
       ...
       //     let image_x = Phaser.Math.Between(100, 400);
       //     this.w_hanako.setPosition(image_x, 100);
       // }
       }
   }
   attack_hanako(player, hanako) {

       this.hanako_Text.setText('痛い!');
       hanako.destroy();
       this.physics.pause();
   ...
   generateHanako() {
       ...
       let image_x = Phaser.Math.Between(200, 400);
       let image_y = Phaser.Math.Between(100, 200);
       this.hanako.create(image_x, image_y, 'hanako');
   }
}